...
\begin{frame}
    \frametitle{Relative Consistency}
\onslide<2-> \begin{block}{Key Point}
    ...
    -\emph{Perfect Set Property} as $\mathsf{L}$ did to ZFC.
\end{block}
\end{frame}

\begin{frame}{Background on PSP}
    \begin{itemize}
        \item<2-> A set of reals is \emph{perfect} iff it is closed and has 
                no isolated points. There are only $2^{\aleph_0}$ such sets.
        \item<3-> A set of reals is said to \emph{have the perfect set property} iff it is 
                countable, or has a perfect subset.
        \item<4-> E.g. every closed set has the PSP.
        \item<5-> If the reals can be well-ordered (AC), not all sets have the PSP.
        \item<6-> The $\Gamma$-PSP is the assertion that every set of reals defined by a 
                formula of type $\Gamma$ has the PSP.
        \item<7-> The $\Gamma$-PSP implies no formula in $\Gamma$ defines a well-order of 
                    the reals.
        \item<8-> $\Gamma$-definable sets are thus `regular' or `well behaved'.
    \end{itemize}
\end{frame}
...
\begin{block}{Intuitive idea}
    ...
    once by a transitive set. (These are the `worlds' in the sense of the interp)
    \item<5->
...
\begin{frame}{Interpretations}
    ...
    \begin{block}{Bi-interpretation}
        ...
        \onslide<5-> We also have a partial converse: 
        
        \emph{for the language without $\dl$},
...
\begin{frame}{Concluding remarks}
    ...
    \item<5-> But I am not convinced that there are decisive reasons for taking one of these, rather than the other, to hold only in inner models. 
    \item<6-> This corresponds, as we have effectively seen in this talk, to not thinking there are decisive reasons to favor pure height over height-and-width potentialism.
    \end{itemize}
\end{frame}
