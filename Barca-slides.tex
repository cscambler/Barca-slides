...
\begin{block}{Bi-interpretation}
    In fact, ZFC $\vdash (\forall X [Tran(X) \rightarrow t(\phi^\diamond)(X)]) \leftrightarrow \phi$;
    \onslide<4->and \\ L$^+$ $\vdash (\forall X[ Tran(X) \rightarrow t(\phi)(X)])^\Diamond \leftrightarrow \phi$
\end{block}
\onslide<5->
This is something like what Button calls near-synonymy (near-near-synonmy, I guess.)
\end{frame}
